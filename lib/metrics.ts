...
export const METRICS: MetricDefinition[] = [
  ...
  { key: 'hourlyCups',         label_zh: '小时杯量',           format: 'count',    comparisons: ['wow', 'mom'],   good_direction: 'up',   source: 'confirmed', formula_zh: '等效商品数 / SUM(t_emp_kpi.attendance_hours) — 数据 T+1 更新（昨日及之前可用）' },
  { key: 'perfHourlyCups',     label_zh: '绩效小时杯量',       format: 'count',    comparisons: ['wow', 'mom'],   good_direction: 'up',   source: 'confirmed', formula_zh: '等效商品数 / SUM(t_emp_kpi.kpi_hours)  // kpi_hours 已排除会议/培训/帮带训/休息 — 数据 T+1 更新（昨日及之前可用）' },
  { key: 'hourlyCupAchieve',   label_zh: '小时杯量达成比',     format: 'percent',  comparisons: ['wow', 'mom'],   good_direction: 'up',   source: 'confirmed', formula_zh: '绩效小时杯量 / 理论小时杯量 (env: THEORETICAL_HOURLY_CUPS, default 30) — 数据 T+1 更新（昨日及之前可用）' },
  { key: 'qcPassRate',         label_zh: '品控稽核达标率',     format: 'percent',  comparisons: ['sequential'],   good_direction: 'up',   source: 'confirmed', formula_zh: 'SUM(t_shopcheck_report.score≥80) / COUNT(*) — 稽核约每月 1 次/店，单日窗口常为空' },
  { key: 'qcAvgScore',         label_zh: '品控稽核平均分',     format: 'score',    comparisons: ['sequential'],   good_direction: 'up',   source: 'confirmed', formula_zh: 'SUM(t_shopcheck_report.score) / COUNT(*) — 稽核约每月 1 次/店，单日窗口常为空' },
